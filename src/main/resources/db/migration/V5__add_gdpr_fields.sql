...
ALTER TABLE users ADD COLUMN is_deleted BOOLEAN DEFAULT FALSE COMMENT 'Soft-delete flag for GDPR compliance';
ALTER TABLE users ADD COLUMN deletion_requested_at DATETIME(6) COMMENT 'Timestamp when user requested deletion';
ALTER TABLE users ADD COLUMN deletion_reason VARCHAR(255) COMMENT 'User-provided reason for deletion';
ALTER TABLE users ADD COLUMN hard_delete_scheduled_at DATETIME(6) COMMENT 'Scheduled hard-delete date (90 days after soft-delete)';
...
CREATE PROCEDURE schedule_gdpr_hard_delete(
    IN p_user_id BIGINT,
    IN p_retention_days INT
)
BEGIN
    UPDATE users 
    SET hard_delete_scheduled_at = DATE_ADD(deletion_requested_at, INTERVAL p_retention_days DAY)
    WHERE id = p_user_id AND is_deleted = TRUE;
END$$

CREATE PROCEDURE perform_gdpr_hard_delete(
    IN p_user_id BIGINT,
...
BEGIN
    ...
    -- Check if user is marked for deletion and retention period has passed
    IF EXISTS (
        SELECT 1 FROM users 
        WHERE id = p_user_id 
        AND is_deleted = TRUE 
        ...
        DELETE FROM transaction_audit_log WHERE user_id = p_user_id;
